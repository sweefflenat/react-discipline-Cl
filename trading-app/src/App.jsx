...
// ═══════════════════════════════════════════════════════════════════════════════
// STYLES - UPDATED FOR IPHONE SCROLL FIX
// ═══════════════════════════════════════════════════════════════════════════════
...
const styles = `
  ...
  * {
  ...
  /* FIXED: Root level scroll unlock */
  html, body {
    ...
    min-height: 100%;
    overflow-x: hidden;
    overflow-y: auto !important; /* Forces system scroll */
    -webkit-overflow-scrolling: touch; /* Momentum scrolling */
  }

  /* FIXED: Prevent nested scroll fighting */
  #root {
    min-height: 100vh;
    overflow-y: visible; 
  }

  /* FIXED: Background must be position: fixed to not block touch */
  .water-bg {
    position: fixed;
    top: 0;
    left: 0;
  ...
  /* FIXED: App padding and height for mobile browser bars */
  .app {
    position: relative;
    z-index: 1;
    min-height: 100vh;
    ...
    padding-bottom: 140px; /* Extra space for the floating nav */
    overflow-y: visible;
  }
  ...
  .mood-circle { width: 58px; height: 58px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 24px; font-weight: 700; flex-shrink: 0; box-shadow: 0 4px 16px rgba(0,0,0,0.08); }
  ...
  .master-filter-icon { font-size: 28px; margin-bottom: 10px; }
  ...
  .streak-label { font-size: 11px; font-weight: 700; letter-spacing: 0.15em; text-transform: uppercase; color: var(--text-light); margin-bottom: 12px; }
  ...
  .metric-label { font-size: 9px; font-weight: 600; letter-spacing: 0.08em; text-transform: uppercase; color: var(--text-light); }
  ...
  .coach-title { font-size: 11px; font-weight: 700; letter-spacing: 0.15em; text-transform: uppercase; color: var(--ocean-deep); margin-bottom: 12px; }
  ...
  .plan-field-label { font-size: 10px; font-weight: 700; letter-spacing: 0.1em; text-transform: uppercase; color: var(--text-muted); margin-bottom: 4px; }
  ...
  .nav-icon { font-size: 22px; margin-bottom: 3px; transition: transform 0.3s ease; }
    ...
    <div className="page" key="premarket">
      ...
      {!masterFilter ? (
        <div className="master-filter locked">
          <div className="master-filter-icon">🔒</div>
          <div className="master-filter-text">
            Acknowledge the master filter to unlock your plan.<br />
            <small>This is your first act of discipline today.</small>
          </div>
        </div>
      ) : (
        <>
          ...
          <div className="card">
            ...
            {traction <= 1 && (
              <div className="alert alert-yellow">
                ...
                Low traction — consider reducing size or stepping aside.
              </div>
            )}
          </div>
          ...
          <div className="card">
            ...
            {cognitiveLoad === 'Overwhelmed' && (
              <div className="alert alert-yellow">
                <span className="alert-icon">⚠️</span>
                Minimize exposure — a stressed mind cannot execute.
              </div>
            )}
      ...
      <div className="card">
        ...
        {followedPlan && mood >= 4 && (
          <div className="alert alert-green">
            ...
            Green day — process followed, mind clear.
          </div>
        )}
        {followedPlan && mood < 4 && (
          <div className="alert alert-yellow">
            <span className="alert-icon">🟡</span>
            Rules followed but felt uneasy.
          </div>
        )}
        {!followedPlan && (
          <div className="alert alert-red">
            <span className="alert-icon">❌</span>
            Plan not followed. Diagnose why.
          </div>
        )}
            ...
            {sortedData.slice(0, 5).map(d => (
              <div key={d.date} className="history-item">
                ...
                <div className="history-meta">
                  ...
                  <span className="history-status">{d.followed_plan ? '✅' : '❌'}</span>
                </div>
              </div>
            ))}
          </div>
...
export default function App() {
  ...
  return (
    <>
      ...
      {/* Background elements are now position: fixed in the patched styles */}
